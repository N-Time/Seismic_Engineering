...
% 4) dT = Natural period Interval (s)
% 5) fig = 0 or 1, Default: 1 = plot the figure
% 6) variable = 'A','V','D' for Acceleration(default),Velocity,Displacement
% 7) normalize = 1,0 for yes,no, Default: 1 for 'A', 0 for 'V' and 'D'
% Output:
% 1) Figure S-T;
% 2) peak_psd = pseudo-;
...
function [peak_abs, peak_psd] = response_spectrum(wave,dt,kesi,abs_psd,dT,fig,variable,normalize)
    ...
    xg=wave*9.80; % transfer from unit g to m/s^2
    xg_peak=max(xg); % PGA
    ...
    for k = 1:length(T)  % for each natural period
    ...
    if normalize == 1
        ...
        peak_psd = var_peak_psd';
        peak_abs = var_peak_abs';
        yL = 'S';
    end
